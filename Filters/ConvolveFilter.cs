...
public class ConvolveFilter : IFilter
{
    ...
    /// <param name="edgeAction"></param>
    public static void Convolve(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, bool alpha,
        EdgeMode edgeAction)
    {
        if (kernel.Height == 1)
    ...
    /// <param name="alpha"></param>
    /// <param name="edgeAction"></param>
    public static void ConvolveHV(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, bool alpha,
        EdgeMode edgeAction)
    {
        var index = 0;
        var matrix = kernel.GetKernel();
        var rows = kernel.Height;
    ...
    /// <param name="alpha"></param>
    /// <param name="edgeAction"></param>
    public static void ConvolveH(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, bool alpha,
        EdgeMode edgeAction)
    {
        var index = 0;
        var matrix = kernel.GetKernel();
    ...
    /// <param name="alpha"></param>
    /// <param name="edgeAction"></param>
    public static void ConvolveV(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, bool alpha,
        EdgeMode edgeAction)
    {
        var index = 0;
        var matrix = kernel.GetKernel();
        ...
        for (var x = 0; x < width; x++)
        {
            ...
            for (var row = -rows2; row <= rows2; row++)
            {
                ...
                var f = matrix[row + rows2];

                if (f != 0)
                {
                    var rgb = inPixels[ioffset + x];
                    a += f * ((rgb >> 24) & 0xff);
                    r += f * ((rgb >> 16) & 0xff);
                    g += f * ((rgb >> 8) & 0xff);
                    b += f * (rgb & 0xff);
