...
public class FastGaussianBlurFilter : GaussianBlurFilter
{
    ...
    public override void Run(KalikoImage image)
    {
        ...
        ConvolveAndTranspose(Kernel, bitmapData, tempBitmapData, bitmapData.Width, bitmapData.Height, UseAlpha,
            UseAlpha && PremultiplyAlpha, false, EdgeMode.Clamp);
        ConvolveAndTranspose(Kernel, tempBitmapData, bitmapData, bitmapData.Height, bitmapData.Width, UseAlpha, false,
    ...
    public static void ConvolveAndTranspose(Kernel kernel, BitmapData inPixels, BitmapData outPixels, int width,
        int height, bool alpha, bool premultiply, bool unpremultiply, EdgeMode edgeAction)
    {
        unsafe
        {
            ...
            Parallel.For(0, height, y =>
            {
                ...
                for (var x = 0; x < width; x++)
                {
                    ...
                    for (var col = -halfKernalWidth; col <= halfKernalWidth; col++)
                    {
                        ...
                        byte sourceA = 0;
                        if (hasAlpha) sourceA = sourceOffset[3];
                    ...
                    if (hasAlpha) destOffset[3] = destinationA;
                    index += height;
                }
            });
        }
