...
public class FastChromaKeyFilter : IFilter
{
    ...
    public FastChromaKeyFilter(Color keyColor, float toleranceHue, float toleranceSaturnation,
        float toleranceBrightness)
    {
        KeyColor = keyColor;
        ...
        unsafe
        {
            ...
            Parallel.For(0, height, y =>
            {
                ...
                for (var x = 0; x < widthInBytes; x = x + bytesPerPixel)
                {
                    ...
                    if (Abs(hsb.Hue, keyHsb.Hue) < ToleranceHue &&
                        Abs(hsb.Saturation, keyHsb.Saturation) < ToleranceSaturnation &&
                        Abs(hsb.Brightness, keyHsb.Brightness) < ToleranceBrightness) currentLine[x + 3] = 0;
                }
            });
